    ...
    for player in ['O', 'X']:
        for (row, col) in empty_squares:
            ...
            if check_win(board, player):
                ...
                return
            board[row][col] = ''  # Reset the board if no immediate win/block

    # If center square is empty, choose it (best starting move)
    if (1, 1) in empty_squares:
        board[1][1] = 'O'
        return
    
    # AI prioritizes corners if available
    corners = [(0, 0), (0, 2), (2, 0), (2, 2)]
    available_corners = [corner for corner in corners if corner in empty_squares]
    if available_corners:
        row, col = random.choice(available_corners)
        board[row][col] = 'O'
        return
    
    # Choose a side square (2, 1) or (1, 2)
    sides = [(0, 1), (1, 0), (1, 2), (2, 1)]
    available_sides = [side for side in sides if side in empty_squares]
    if available_sides:
        row, col = random.choice(available_sides)
        board[row][col] = 'O'
 
        return

    # If no preferred moves are available, choose any empty square
